...
theta_A = np.random.normal(loc = new_A, size = n_prior2)
theta_B = np.random.normal(loc = new_B, size = n_prior2)
posterior_A, posterior_B= [],[]
for each in theta_A:
    for every in theta_B:
        x_sim,data_sim = simulate(each, every, n = n_sim,noise = False)
...
p1.scatter(posterior_B,posterior_A)
